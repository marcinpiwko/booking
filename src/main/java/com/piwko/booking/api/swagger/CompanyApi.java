...
public interface CompanyApi {
    ...
    ResponseEntity<GetCompaniesForm> getCompanies(@RequestParam Optional<Integer> page, @RequestParam Optional<Integer> size, @RequestParam Optional<String> sortBy, @RequestParam Optional<String> city, @RequestParam Optional<String> specializationCode);

    @Operation(summary = "Create new company", description = "Create new company", tags = "companies", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
    ...
    ResponseEntity<IdResponse> createCompany(@Valid @RequestBody PostCompanyForm postCompanyForm) throws ResourceNotUniqueException, ResourceNotFoundException;

    @Operation(summary = "Modify company for specified id", description = "Modify company for specified id", tags = "companies", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
    ...
    @Operation(summary = "Delete company for specified id", description = "Delete company for specified id", tags = "companies", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "No content"),
            @ApiResponse(responseCode = "404", description = "Not found", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    ...
    ResponseEntity<Void> deleteCompany(@PathVariable("id") Long id) throws ResourceNotFoundException;

    @Operation(summary = "Refresh company cache", description = "Refresh company cache", tags = "companies", security = @SecurityRequirement(name = "bearerAuth"))
    @ApiResponses(value = { @ApiResponse(responseCode = "204", description = "No content")})
    @PreAuthorize("hasRole('ROLE_ADMIN')")
    @PostMapping(value = "/refresh")
    ResponseEntity<Void> refreshCompanies();
}
